...
export class GetAddressInfoByHash extends Action {
  ...
  async run({ params }) {
    ...
    if ((params.hash).charAt(0) == X_CHAIN) {
      addressInfoFromXChain = await xChain.getAddressInfoByHashFromXChain(params.hash);
      ...
      addressInfoFromPChain = await pChain.getAddressInfoFromPChain(params.hash);
      ...
      addressInfoFromCChain = await cChain.getAddressInfoFromCChain(params.hash);
